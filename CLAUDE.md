...
- **5.5d status (chr20).** End-to-end with both fixes: **210 PASS-flips of 209556 sites (0.10 %), 697 total FILTER mismatches (0.33 %).** Down from 535/2367 (1.13 %) at the start of the phase. Wall-time 3:10 m:s on M4 Max with 14 threads (matches the threaded-no-fix baseline). Pileup-DP comparison: 98.4 % of sites have IDENTICAL DP between us and Docker. The remaining ~0.6 % (1259 sites) where ours DP > 3 × docker DP correspond to anomalously high-coverage regions (chr20:31185803 has 5049 raw reads in the BAM); Docker appears to apply some position-cap or downsampling that we don't, but the impact on FILTER classification is minimal at most other sites.
- **5.5e — extension to all variants.** Pending. Remaining residual drift is plausibly a mix of (a) high-coverage-site DP differences, (b) cross-MID dispatch differences (small_model vs deepvariant), and (c) FP32 reduction-order differences between Apple GPU and x86 AVX-512+FMA — the last of which is fundamentally bounded by hardware (per Phase 5.5c — replacing the stem with deterministic Metal kernels gives 100 % identical FILTER output to MPSGraph, so MPSGraph-vs-TF drift is the irreducible floor).
