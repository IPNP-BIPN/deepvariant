...
- **5.5d status (chr20, FINAL).** End-to-end with all eight fixes: **0 FILTER mismatches of 210376 shared sites, 0 PASS-flips, identical PASS sets (107113 = 107113)**. Down from 535/2367 (1.13 %) at the start of the phase. **100 % FILTER parity vs google/deepvariant:1.10.0 Docker on shared sites of chr20.** Wall-time 3:13 m:s on M4 Max with 14 threads (small_model now scalar FP32 CPU; ~12 s overhead). Remeasured 2026-04-29: 14 only-ours / 14 only-docker, all RefCall, all at saturated multi-allelic sites — same position both sides, different alt allele kept. Root cause: BNNS-CPU softmax FP drift vs TF/Keras Eigen-x86. At saturated `predictions[0] ≈ 1.0` sites, upstream's `compute_quals` returns `-10·log10(1-sum_alt) ≈ 1e-7`, and `get_alt_alleles_to_remove`'s max picks the alt whose tiny qual delta is highest by FP rounding. Our scalar BNNS-CPU produces identical-to-1.0 `predictions[0]` (form A returns 0 → first-iteration wins) while Docker's vectorised Eigen produces sub-ULP-different values per alt (form B picks based on FP order). On 14 sites the relative qual order differs from Docker's. Fundamentally unfixable without HW-emulating x86 SIMD reduction order. AD pattern across the 14 is mixed (sometimes Docker picks higher AD, sometimes lower, sometimes tied) — confirms the choice is genuine FP arithmetic, not a heuristic. **No functional impact**: both ours and Docker call homref (RefCall) at every one of these positions. PASS variant set is identical.
