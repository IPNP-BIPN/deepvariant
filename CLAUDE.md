...
- **Phase 9 / Steps 1, 2a, 5a — DV-base feature completion (in progress).** ✅ Steps 1+2a+5a DONE 2026-05-01 (3 commits). User directive: stick to base DeepVariant only — no DeNovoCNN, no VEF, no ensemble. Five Phase 9 items extend native port to full upstream parity (alt-aligned pileup, methylation, gVCF, DirectPhasing, whole-genome F1). Status:
  ...
  - **Step 3 (gVCF block emission) deferred** — ~620 LOC across make_examples (ref-confidence TFRecord emission), postprocess (band-coalescing + gVCF writer), and reuse of `nucleus/io/merge_variants.cc::TransformToGvcf()`.
  - **Step 4a — DirectPhasing link + flag** ✅ done 2026-05-01 (commit 236ae036). `dv_direct_phasing` linked into `dv_make_examples_lib` in `CMakeLists.txt`. New `ABSL_FLAG(use_direct_phasing, false)` in make_examples_main.cc (default false → baseline preserved). The library + algorithm are fully present; only the per-region orchestration is missing. Step 4b (the orchestration: per-region instantiation + PhaseReads call + GetPhasedVariants walkback + candidate.call.is_phased/info["PS"] assignment + cross-region stitching) is deferred — ~300 LOC across the multi-sample worker loop; needs careful state-tracking refactor.
  - **Step 5b (whole-genome data download + run) deferred** — ~120 GB BAM + 3.1 GB GRCh38 + truth sets for HG002/HG003/HG004 (~150 MB total) + ~9 h of wall-time across 3 samples.
