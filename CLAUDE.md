...
- **Phase 9 / Steps 1, 2a, 5a — DV-base feature completion (in progress).** ✅ Steps 1+2a+5a DONE 2026-05-01 (3 commits). User directive: stick to base DeepVariant only — no DeNovoCNN, no VEF, no ensemble. Five Phase 9 items extend native port to full upstream parity (alt-aligned pileup, methylation, gVCF, DirectPhasing, whole-genome F1). Status:
  ...
  - **Step 4 (DirectPhasing wiring → phased GT + PS field) deferred** — ~330 LOC. Upstream `dv_direct_phasing` library exists and is fully featured (Boost-graph max-weight phasing); needs CMake link + per-region instantiation in `make_examples_main.cc::ProcessRegion()` + write-back of phasing results to candidate `VariantCall.is_phased + info["PS"]` before TFRecord emission.
