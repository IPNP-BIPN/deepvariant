...
- **5.5d status (chr20, FINAL — 2026-04-29).** End-to-end with all nine fixes: **210390/210390 site-set parity (100 %), 0 FILTER mismatches, 107113/107113 PASS variants identical**. Wall-time 3:14 m:s on M4 Max with 14 threads. Remaining record-level diffs (88.3 % of records byte-identical to Docker, 11.7 % differ): PL field drift on ~9 % of records (Inception-v3 FP32 cumulative drift over 188 layers — explicit non-goal per plan; documented as "fundamentally unachievable on Apple GPU due to FP32 non-associativity in any parallel reduction"); QUAL ±0.1 on ~2.3 % (same root cause); MID annotation `small_model` vs `deepvariant` on ~0.3 % (small_model dispatch threshold lands on opposite sides of GQ boundary due to softmax drift, both paths give same final GT/FILTER). **Zero records differ in CHROM/POS/REF/ALT, FILTER, or GT** — every user-facing genomic conclusion matches Docker on chr20.
