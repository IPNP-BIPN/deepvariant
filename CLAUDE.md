...
- **5.5d status (chr20, FINAL).** End-to-end with all eight fixes: **0 FILTER mismatches of 210376 shared sites, 0 PASS-flips, identical PASS sets (107113 = 107113)**. Down from 535/2367 (1.13 %) at the start of the phase. **100 % FILTER parity vs google/deepvariant:1.10.0 Docker on shared sites of chr20.** Wall-time 3:24 m:s on M4 Max with 14 threads (small_model now scalar FP32 CPU; ~12 s overhead). Remaining 14 / 14 only-ours / only-docker sites are candidate-emission edge cases (boundary alleles below threshold, etc.) — total VCF size is conserved (210390 = 210390); investigated separately.
