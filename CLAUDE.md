...
- **Phase 9 / Steps 1, 2a, 5a — DV-base feature completion (in progress).** ✅ Steps 1+2a+5a DONE 2026-05-01 (3 commits). User directive: stick to base DeepVariant only — no DeNovoCNN, no VEF, no ensemble. Five Phase 9 items extend native port to full upstream parity (alt-aligned pileup, methylation, gVCF, DirectPhasing, whole-genome F1). Status:
  ...
  - **Step 3 — gVCF block emission stub + warning** ⏸ implementation deferred (commit e6ac09d4). Adds runtime warning when `--gvcf_outfile` is set so users don't silently get no file. Detailed TODO inline in postprocess_main.cc: ~300-400 LOC port of upstream's Python `_calls_and_gvcfs` (variant_calling.cc has zero gVCF support; refs are Python-only) + non-variant TFRecord I/O + band-coalescing + 2nd VcfWriter with END field. Multi-day focused work; not a clean session unit.
