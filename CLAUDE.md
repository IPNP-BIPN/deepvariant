...
- **Phase 9 / Steps 1, 2a, 5a — DV-base feature completion (in progress).** ✅ Steps 1+2a+5a DONE 2026-05-01 (3 commits). User directive: stick to base DeepVariant only — no DeNovoCNN, no VEF, no ensemble. Five Phase 9 items extend native port to full upstream parity (alt-aligned pileup, methylation, gVCF, DirectPhasing, whole-genome F1). Status:
  ...
  - **Step 3 — gVCF block emission** ✅ done 2026-05-01. New `deepvariant/native/gvcf_emit.{h,cc}` (~190 LOC) ports upstream's `make_gvcfs` from `variant_caller.py:256-410` to C++: per-site reference-confidence (log10[ref/het/alt] from `n_ref`/`n_total`/`p_error`), Phred GQ from `(1 - p_ref)`, GQ-banding via `floor(raw/binsize)*binsize`, and consecutive-position group merge into one Variant with `<*>` alt + `END` info + min_gq + min_dp. New `--gvcf` flag in make_examples_main.cc spawns a per-thread sharded TFRecord writer (`gvcf.tfrecord@N`) that consumes `probe.SummaryCounts(0,0)` per region (no gating on candidate presence — every position gets a row). New `--nonvariant_site_tfrecord_path` flag in postprocess_main.cc; when `--gvcf_outfile` is set, postprocess writes its post-haplotype-resolution variants to a temp TFRecord and hands both streams to upstream's `nucleus::MergeAndWriteVariantsAndNonVariants` (lower-level signature) which walks them in coordinate order, applies `TransfromToGvcf` to each variant (adds `<*>` to alt list + `0` to AD/VAF), and emits VCF + gVCF in lockstep. cli.cc plumbs `--output_gvcf` → `--gvcf=<tmp>/gvcf.tfrecord@N` for make_examples → `--gvcf_outfile + --nonvariant_site_tfrecord_path` for postprocess. Header gains `MIN_DP`/`MED_DP` FORMAT declarations (no-op when gVCF disabled). Verified chr20:10M-10.1M: with `--output_gvcf` produces 3247-row gVCF (313 variant rows with `<*>`-extended alt + 2934 reference-confidence blocks, every position in [10000000, 10100000] covered, zero gaps); WITHOUT the flag the VCF is byte-identical to the pre-Step-3 baseline (313/313 records match, header diff empty). Default off — production baseline preserved.
