...
- **5.5d status (chr20).** End-to-end with all three fixes: **27 PASS-flips of 209814 shared sites (0.013 %), 29 total FILTER mismatches (0.014 %)**. Down from 535/2367 (1.13 %) at the start of the phase. Wall-time 3:15 m:s on M4 Max with 14 threads (negligible reservoir-sampling overhead). Threading commit + Phase 5.5c + 5.5d/{1,2,3} = effectively at FILTER parity vs google/deepvariant:1.10.0 Docker.
- **5.5e — extension to all variants.** Pending. Remaining 27 PASS-flips are likely a mix of (a) one-sided cross-MID dispatch edge cases (we PASS via small_model, Docker RefCall via deepvariant), and (b) FP32 reduction-order differences between Apple GPU and x86 AVX-512+FMA on the rare softmax-near-threshold sites — the latter is fundamentally bounded by hardware (per Phase 5.5c, replacing the stem with deterministic Metal kernels gives 100 % identical FILTER output to MPSGraph, so MPSGraph-vs-TF drift is the irreducible floor).
